...
SELECT 'IF (SUSER_ID('+QUOTENAME(SP.name,'''')+') IS NULL) BEGIN CREATE LOGIN ' +QUOTENAME(SP.name)+
			   CASE 
					WHEN SP.type_desc = 'SQL_LOGIN' THEN ' WITH PASSWORD = ' +CONVERT(NVARCHAR(MAX),SL.password_hash,1)+ ' HASHED, SID=' + convert(VARCHAR(MAX),SL.SID,1 ) + ', CHECK_EXPIRATION = ' 
...
SELECT 
	CASE WHEN SrvPerm.state_desc <> 'GRANT_WITH_GRANT_OPTION' 
		...
		ELSE 'GRANT' 
	END
    + ' ' + SrvPerm.permission_name + CASE WHEN SrvPerm.grantor_principal_id>1 then ' ON LOGIN::['+ SP2.name + ']' else '' end + ' TO [' + SP.name + ']' + 
	CASE WHEN SrvPerm.state_desc <> 'GRANT_WITH_GRANT_OPTION' 
		THEN '' 
...
FROM sys.server_permissions AS SrvPerm 
	...
	JOIN sys.server_principals AS SP2 ON SrvPerm.grantor_principal_id = SP2.principal_id 
