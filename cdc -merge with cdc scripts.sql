...
--step1 : enable CDC at database level
USE[CTWQ]
...
--step2: enable CDC on all tables one by one
...
--step3: It will create few SQL agent jobs to run CDC
 
 
-- followings are the queries to monitor/watch CDC
...
--run these queries to see data change records, AB and DB are the tables name, replace your Table
select * from cdc.dbo_FL_CT
--select * from cdc.dbo_<table_name>_CT
 
 
--grant access to the users (this is for Databricks only)
GRANT SELECT ON object::sys.schemas TO  svc_azuredf;
GRANT SELECT ON object::sys.tables TO  svc_azuredf;
GRANT SELECT ON object::sys.columns TO  svc_azuredf;
GRANT SELECT ON object::sys.key_constraints TO  svc_azuredf;
GRANT SELECT ON object::sys.foreign_keys TO  svc_azuredf;
GRANT SELECT ON object::sys.check_constraints TO  svc_azuredf;
GRANT SELECT ON object::sys.default_constraints TO  svc_azuredf;
GRANT SELECT ON object::sys.change_tracking_tables TO  svc_azuredf;
GRANT SELECT ON object::sys.objects TO  svc_azuredf;
GRANT SELECT ON object::sys.triggers TO  svc_azuredf;
GRANT SELECT ON object::sys.indexes TO  svc_azuredf;
GRANT SELECT ON object::sys.index_columns TO  svc_azuredf;
GRANT SELECT ON object::sys.fulltext_index_columns TO  svc_azuredf;
GRANT SELECT ON object::sys.fulltext_indexes TO  svc_azuredf;
GRANT SELECT ON schema::dbo TO  svc_azuredf;
GRANT SELECT ON object::AB TO  svc_azuredf;

--verify cdc enabled databases
...
BEGIN
    ...
    SET @mode = 'CDC'; -- ** SET THE MODE HERE ** ['BOTH'|'CT'|'CDC'|'NONE']

    DECLARE @replicationUser NVARCHAR(100);
    SET @replicationUser = 'svc_azuredf'; -- ** SET THE USERNAME HERE **
        ...
        BEGIN;
            ...
            BEGIN TRY
                ...
                EXECUTE('
                    ...
                    BEGIN
                        INSERT INTO [' + @DbName + '].dbo.[' + @ddlAuditTableName + '] (
                            [CURRENT_USER],
                            [SCHEMA_NAME],
                            [TABLE_NAME],
                            [TYPE],
